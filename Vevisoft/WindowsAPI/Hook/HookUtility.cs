 ...
 *文件名：  HookUtility
 *版本号：  V1.0.0.0
 *唯一标识：626c3901-2ec6-4352-9955-f6cc769869e0
 *当前的用户域：VEVISOFT
 *创建人：  vevi
 *电子邮箱：
 *创建时间：2014/12/1 16:05:21
 *描述：
 *
 *=====================================================================
 *修改标记
 *修改时间：2014/12/1 16:05:21
 *修改人： Administrator
 *版本号： V1.0.0.0
 *描述：
 *
...
namespace Vevisoft.WindowsAPI.Hook
{
    public class HookUtility
    {
        ...
        public static extern IntPtr SetWindowsHookEx(int idHook, HookUtility.HookProc lpfn, IntPtr hInstance, int threadId);
        //卸下钩子的函数 
        [DllImport("user32.dll ", CharSet = CharSet.Auto, CallingConvention = CallingConvention.StdCall)]
        ...
        IntPtr MyHookProc(int code, int wparam, IntPtr lparam)
        {
        ...
        public void SetHook()
        {
            ...
            var myhookProc = new HookProc(MyHookProc); //声明一个自己的Hook实现函数的委托对象

            _nextHookPtr = SetWindowsHookEx((int)HookType.WH_CBT, myhookProc, IntPtr.Zero, 0);// GetCurrentThreadId()); //加到Hook链中
        }
        //卸载HOOK
        public void UnHook()
        {
